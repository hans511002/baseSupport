...
import com.sobey.jcg.support.utils.Bytes;
...
public class HashTable extends HeapSize implements JudgeHash {
	...
	public static final int HashHeadSize = jdkByte + jdkByte + RoundJdkByte(Integer.SIZE * 2 / Byte.SIZE) + RoundJdkByte(Long.SIZE / Byte.SIZE)
	        + RoundJdkByte(Byte.SIZE / Byte.SIZE);
	...
	public String analysis() {
		...
		sb.append("索引重复数：" + this.duplicateSize + "  占比：" + (double) this.duplicateSize / (this.duplicateSize + this.onceSize) + " 内存占用：" + this.heapSize
		        + "\n");
	...
	public void writeToFile(String file, int mulSize) throws IOException {
		...
		System.out.println("落地HashTable到文件 file=" + fl.getCanonicalPath() + "  mapFileSize=" + fileSize + " hashSize=" + this.nodes.length + "  duplicateSize="
		        + this.duplicateSize);
					...
					if (mulSize <= 1) {
						...
						while (_node.next != 0) {
							...
							try {
								...
								e.printStackTrace();
								System.err.println("startIndex=" + startIndex + "> mapFileSize=" + mapFileSize + " _node.next=" + _node.next);
