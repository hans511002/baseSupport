...
public class AopFactory implements Opcodes {
	...
	private static byte[] getSubClassByte(Class<?> sourceClass, String[] methodIncludes, IAopMethodFilter filter) {
		...
		for (MethodNode methodNode : methods) {
			...
			// 访问beforeHander方法。
			visitor.visitMethodInsn(INVOKEVIRTUAL, invokeType.getInternalName(), "beforeHanderInvoke",
	...
	private static List<MethodNode> getMethodNodes(Class<?> sourceClass) {
		try {
			...
			InputStream is = sourceClass.getClassLoader().getResourceAsStream(
					sourceClass.getCanonicalName().replace(".", "/") + ".class");
	...
	private static boolean boxUp(Type type, MethodVisitor visitor) {
		switch (type.getSort()) {
		case Type.BOOLEAN: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Boolean.class).getInternalName(), "valueOf",
					"(Z)Ljava/lang/Boolean;");
			break;
		}
		case Type.INT: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Integer.class).getInternalName(), "valueOf",
					"(I)Ljava/lang/Integer;");
			break;
		}
		case Type.CHAR: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Character.class).getInternalName(), "valueOf",
					"(C)Ljava/lang/Character;");
			break;
		}
		case Type.SHORT: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Short.class).getInternalName(), "valueOf",
					"(S)Ljava/lang/Short;");
			break;
		}
		case Type.BYTE: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Byte.class).getInternalName(), "valueOf",
					"(B)Ljava/lang/Byte;");
			break;
		}
		case Type.LONG: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Long.class).getInternalName(), "valueOf",
					"(J)Ljava/lang/Long;");
			break;
		}
		case Type.FLOAT: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Float.class).getInternalName(), "valueOf",
					"(F)Ljava/lang/Float;");
			break;
		}
		case Type.DOUBLE: {
			visitor.visitMethodInsn(INVOKESTATIC, Type.getType(Double.class).getInternalName(), "valueOf",
					"(D)Ljava/lang/Double;");
			break;
		}
		default:
