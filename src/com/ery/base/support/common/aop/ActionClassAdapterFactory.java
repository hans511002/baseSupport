...
public class ActionClassAdapterFactory implements Opcodes {
	...
	public Class<?> getInterface(Class<?> clazz) throws ClassNotFoundException{
		...
		if(loader.contains(convertClassName(interfaceClassName))){
			return loader.findClass(convertClassName(interfaceClassName));
		}
		...
		writer.visit(V1_1, ACC_PUBLIC+ACC_ABSTRACT+ACC_INTERFACE, interfaceClassName, null, "java/lang/Object",null);
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if(method.getModifiers()==Modifier.PUBLIC){
				MethodVisitor visitor = writer.visitMethod(ACC_PUBLIC+ACC_ABSTRACT, method.getName(),Type.getMethodDescriptor(method), null, null);
	...
	public Class<?> getAdapter(Class<?> clazz,String interfaceName) throws ClassNotFoundException{
		...
		if(loader.contains(convertClassName(childClassName))){
			return loader.findClass(convertClassName(childClassName));
		}
		...
		writer.visit(V1_1, ACC_PUBLIC, childClassName, null, className, new String[]{classNameConvert(interfaceName)});
