...
import com.ery.base.support.jdbc.AsciiStream;
import com.ery.base.support.jdbc.BinaryStream;
import com.ery.base.support.jdbc.CharacterStream;
...
public class TestDAO extends BaseDAO {
	...
	public String queryForString() {
		return super.getDataAccess().queryForString(
				"SELECT CODE_ITEM  FROM  META_SYS_CODE A WHERE A.CODE_ID =null AND 1=?", 1);
	}
	...
	public void exeUpdate() {
		...
		try {
			Object[] params = new Object[] {
					...
					new BinaryStream(new ByteArrayInputStream("ABCDEFG".getBytes())),// blob 二进制
					new AsciiStream(new FileInputStream("D:\\tt.txt")),// clob ASCII字符
					new CharacterStream(new BufferedReader(new InputStreamReader(new ByteArrayInputStream(
							"测试测试".getBytes()), "UTF-8"))) // nclob,可自定义字符集
			};
	...
	public int execQueryCall() {
		...
		ResultSet rs = null;
		try {
			CallableStatement statement = super.getDataAccess().execQueryCall(sql,
					new DBOutParameter(OracleTypes.CURSOR));
			rs = (ResultSet) statement.getObject(1);
			if (rs.next()) {
				CLOB clob = (CLOB) rs.getClob(8);
				System.out.println(clob.stringValue());
			}
	...
	public MeteTestPO queryByRowMapper(int userId) {
		String sql = "SELECT A.USER_ID,LOG_ID,CHANGE_TIME,CHANGE_TYPE,EDITOR_TYPE,EDITOR_ID FROM META_MAG_TEST A WHERE A.USER_ID = ? ";
		return getDataAccess().queryByRowMapper(sql, new BeanMapper<MeteTestPO>(MeteTestPO.class),
				new Object[] { userId });
	}
